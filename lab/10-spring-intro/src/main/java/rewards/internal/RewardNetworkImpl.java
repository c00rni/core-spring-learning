 ...
 * TODO-05: Review the RewardNetworkImpl configuration logic (Refer to the lab
 * document)
...
public class RewardNetworkImpl implements RewardNetwork {
	...
	public RewardConfirmation rewardAccountFor(Dining dining) {
		...
		Account custormerAccount = this.accountRepository.findByCreditCard(dining.getCreditCardNumber());
		Restaurant restaurant = this.restaurantRepository.findByMerchantNumber(dining.getMerchantNumber());
		MonetaryAmount amountsave = restaurant.calculateBenefitFor(custormerAccount, dining);
		AccountContribution contribution = custormerAccount.makeContribution(amountsave);
		this.accountRepository.updateBeneficiaries(custormerAccount);
